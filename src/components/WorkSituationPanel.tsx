...
const planeData = [
  { name: '运12', value: 25, color: '#3b82f6' },
  { name: '空中国王', value: 18, color: '#fcd34d' },
  { name: '新舟60', value: 5, color: '#84cc16' },
  { name: '大中型无人机', value: 5, color: '#ef4444' },
  { name: '运7', value: 2, color: '#0ea5e9' },
];
...
const GlassCapsuleBar = (props: any) => {
  ...
  return (
    <g>
      ...
      <rect x={x} y={y} width={width} height={height} fill={fill} rx={width/2} opacity={0.4} />
      
      {/* Inner Glow / Highlight */}
      <rect x={x + 1} y={y + 1} width={width - 2} height={height - 2} fill="transparent" stroke="rgba(255,255,255,0.15)" strokeWidth={1} rx={(width-2)/2} />
      
      {/* Top Edge Bright Highlight */}
      <path d={`M ${x + 2} ${y + 2} Q ${x + width/2} ${y} ${x + width - 2} ${y + 2}`} fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth={2} strokeLinecap="round" />
...
export function WorkSituationPanel() {
  ...
  const markers = useMemo(() => {
    ...
    return Array.from({ length: count }).map((_, i) => {
      ...
      const baseCity = chinaCities[Math.floor(Math.random() * chinaCities.length)];
    ...
    <div className="absolute inset-0 flex gap-6 p-6 bg-transparent">
      ...
      <div className="flex-[2] flex flex-col gap-4 relative">
        <div className="flex-1 glass-panel ios-squircle mirror-edge relative overflow-hidden group">
          ...
          <div className="absolute top-4 left-4 z-20 flex gap-2 bg-white/5 p-1.5 rounded-xl border border-white/10 backdrop-blur-md shadow-2xl">
            <button 
              className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all duration-500 ${activeMap === 'plane' ? 'bg-cyan-500/40 text-cyan-100 shadow-[0_0_15px_rgba(34,211,238,0.6)]' : 'text-slate-400 hover:text-cyan-300 hover:bg-white/5'}`}
          ...
          <div className="absolute bottom-4 right-4 bg-white/5 border border-white/10 p-2 rounded-xl text-[10px] text-slate-300 z-20 flex gap-4 shadow-2xl backdrop-blur-xl scale-90 origin-bottom-right">
            {activeMap === 'plane' ? (
              <>
      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        <div className="flex-1 glass-panel ios-squircle mirror-edge p-4 flex flex-col relative liquid-border-anim">
          <h3 className="text-lg font-bold refractive-text mb-4 text-center tracking-widest">全国飞机型号和数量</h3>
            ...
            <div className="w-full h-full relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ left: '-30%' }}>
                <div className="flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold text-cyan-100 drop-shadow-[0_0_10px_rgba(34,211,238,0.8)]">55</span>
                  <span className="text-[10px] text-cyan-200/70">总架次</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 glass-panel ios-squircle mirror-edge p-4 flex flex-col relative liquid-border-anim">
          <h3 className="text-lg font-bold refractive-text mb-4 text-center tracking-widest">全国地面作业装备数量</h3>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={equipmentData} margin={{ top: 20, right: 10, left: -10, bottom: 0 }}>
                ...
                <Bar dataKey="高炮" fill="#60a5fa" barSize={16} name="高炮 (门)" shape={<GlassCapsuleBar />} />
                <Bar dataKey="火箭" fill="#bae6fd" barSize={16} name="火箭 (架)" shape={<GlassCapsuleBar />} />
                <Bar dataKey="烟炉" fill="#fde047" barSize={16} name="烟炉 (台)" shape={<GlassCapsuleBar />} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
